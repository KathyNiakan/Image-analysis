...
create_expression_heatmap <- function(expression_matrix, metadata, genes_of_interest) {
  ...
  breaks <- seq(-2, 2.5, length.out = 10)
  ...
  pheatmap(
    ...
    color = inferno(10),
    breaks = breaks,
    filename = "EPI_heatmap.png"
  )
}

epi_deg_OI <- (read.csv("EPI_goi.csv", header = FALSE))
#epi_deg_OI <- (read.csv("EPI_othergoi.csv", header = FALSE))
...
# Assign as rownames
rownames(ICM_counts) <- unique_symbols

# Drop extra columns
ICM_counts <- ICM_counts[, !(colnames(ICM_counts) %in% info)]
...
ICM_deg_OI <- (read.csv("ICM_goi.csv", header = FALSE))
